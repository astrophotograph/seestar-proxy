...
async fn main() -> anyhow::Result<()> {
    ...
    if config.raw {
        ...
        println!(
            ...
            upstream_control, upstream_imaging
        );
    ...
    println!(
        "  Upstream:   {}:{} (control), {}:{} (imaging)",
        upstream_ip, config.upstream_control_port, upstream_ip, config.upstream_imaging_port,
    );
    println!(
        "  Listening:  {} (control), {} (imaging)",
        bind_control, bind_imaging
    ...
    let imaging_handle = tokio::spawn(async move {
        if let Err(e) =
            imaging::run(bind_imaging, upstream_imaging, recorder_i, Some(metrics_i)).await
        {
